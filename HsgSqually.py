...
class HsgSqually:
    ...
    async def move_to_target_position_straight_line(
        self, x_pos_target, y_pos_target, angle_target
    ):
        distance = calculate_distance(
            self.current_x_pos, self.current_y_pos, x_pos_target, y_pos_target
        )
    ...
    async def say(self, message={"role": "failure", "content": "An error occured."}):
    ...
    async def chat(self):
        while True:
            ...
            if response["content"] == "Picture taken!":
                frame = await self.robot.next_frame()
                await self.AI_module.get_response(frame)
